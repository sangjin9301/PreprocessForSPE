...
class IndexMaker {
    ...
    for( i <- 0 to chart.size-1){
      ...
      for( j <- 0 to 8){
        try{
          ...
          case e:NullPointerException => list_9Days.add(chart.get(i).getEndPrice)
        }
      }
    ...
    for( i <- 0 to chart.size-1){
      ...
      for( j <- 0 to 25){
        try{
          ...
          case e:NullPointerException => list_26Days.add(chart.get(i).getEndPrice)
        }
      }
  ...
  def TrailingSpan(standard:util.LinkedList[Double],conversion:util.LinkedList[Double]):util.LinkedList[Double]={
    ...
    for( i <- 25 to span.size -1 ){
      trailingSpan.add(span.get(i))
    }
    return trailingSpan
  }
